...
public class AdrenalineSystemHandler {
    ...
    public static void onPlayerTick(PlayerTickEvent.Post event) {
        if (event.getEntity() != null) {
            handleAdrenalineCharging(event.getEntity());
            handleAdrenalineDecay(event.getEntity());
            handleAdrenalineConsumption(event.getEntity());
            handleAdrenalineFullEffect(event.getEntity());
            handleAdrenalineFullSound(event.getEntity());
        }
    }
    ...
    public static void handleAdrenalineCharging(Player player) {
        ...
        // 如果肾上腺素效果正在激活，不进行充能
        if (player.hasEffect(RevengeanceModMobEffects.ADRENALINE_EFFECT)) {
            return;
        }
        ...
        if (hasBossNearby(player)) {
            ...
            if (currentAdrenaline < MAX_ADRENALINE) {
                ...
                player.getAttribute(RevengeanceModAttributes.ADRENALINE_LEVEL).setBaseValue(newValue);
            } else {
                // 已充满，持续给予抗性提升2效果
    ...
    public static void activateAdrenaline(Player player) {
        ...
        // 检查是否已满且没有激活效果
        if (currentAdrenaline >= MAX_ADRENALINE && !player.hasEffect(RevengeanceModMobEffects.ADRENALINE_EFFECT)) {
            if (!player.level().isClientSide()) {
                // 给予肾上腺素效果
                player.addEffect(new MobEffectInstance(RevengeanceModMobEffects.ADRENALINE_EFFECT,
    ...
    private static void handleAdrenalineLossOnHurt(Player player, float damage) {
        ...
        if (currentAdrenaline >= MAX_ADRENALINE) {
            player.getAttribute(RevengeanceModAttributes.ADRENALINE_LEVEL).setBaseValue(0);
