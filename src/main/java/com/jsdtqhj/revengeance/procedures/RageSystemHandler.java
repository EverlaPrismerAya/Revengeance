...
public class RageSystemHandler {
    ...
    public static void onPlayerTick(PlayerTickEvent.Post event) {
        if (event.getEntity() != null) {
            // 定期清理过期的缓存
            cleanupExpiredCache(event.getEntity().level().getLevelData().getGameTime());
            handleRageConsumption(event.getEntity());
            chargeRageForPlayer(event.getEntity());
            handleRageAnimation(event.getEntity());
    ...
    public static void onMobEffectExpired(MobEffectEvent.Expired event) {
        // 添加调试日志
        RevengeanceMod.LOGGER.info("MobEffectEvent.Expired triggered for effect: {}", event.getEffectInstance().getEffect());
        
        // 检查是否为怒气效果被移除
        if (event.getEffectInstance().getEffect() == RevengeanceModMobEffects.RAGE_EFFECT) {
            LivingEntity entity = event.getEntity();
            RevengeanceMod.LOGGER.info("Rage effect expired for entity: {}", entity.getName().getString());
            
            // 播放怒气结束音效
            if (!entity.level().isClientSide() && entity.level() instanceof ServerLevel serverLevel) {
                RevengeanceMod.LOGGER.info("Playing rage end sound");
    ...
    public static void resetAttributes(Entity entity) {
        ...
        // 重置怒气值
        if (living.getAttributes().hasAttribute(RevengeanceModAttributes.RAGE_LEVEL)) {
            living.getAttribute(RevengeanceModAttributes.RAGE_LEVEL).setBaseValue(0);
            // 同时清除相关标记
        ...
        // 重置肾上腺素值
        if (living.getAttributes().hasAttribute(RevengeanceModAttributes.ADRENALINE_LEVEL)) {
            living.getAttribute(RevengeanceModAttributes.ADRENALINE_LEVEL).setBaseValue(0);
        }
    }

    // 创建一个静态的TagKey，避免重复创建
    private static final ResourceLocation RAGE_TAG_LOCATION = 
            ResourceLocation.parse(RevengeanceMod.MODID + ":not_trigger_rage");
    private static final TagKey<EntityType<?>> NOT_TRIGGER_RAGE = 
            TagKey.create(BuiltInRegistries.ENTITY_TYPE.key(), RAGE_TAG_LOCATION);
    ...
    public static void chargeRageForPlayer(Player player) {
        // 检查玩家是否有怒气属性
        if (!player.getAttributes().hasAttribute(RevengeanceModAttributes.RAGE_LEVEL)) {
            return;
        }

        // 如果怒气效果正在激活，不进行充能
        if (player.hasEffect(RevengeanceModMobEffects.RAGE_EFFECT)) {
            return;
        }
        ...
        double current = player.getAttribute(RevengeanceModAttributes.RAGE_LEVEL).getValue();

        if (totalChargeRate > 0) {
            // 有敌对生物在范围内，进行充能
            double newValue = Math.min(MAX_RAGE, current + totalChargeRate / 20.0); // 除以20因为每tick调用
            
            player.getAttribute(RevengeanceModAttributes.RAGE_LEVEL).setBaseValue(newValue);
    ...
    public static void activateRage(Entity entity) {
        ...
        if (!living.getAttributes().hasAttribute
                (RevengeanceModAttributes.RAGE_LEVEL)) {
            return;
        }

        // 检查怒气值是否达到100且没有激活怒气效果
        if (living.getAttribute(RevengeanceModAttributes.RAGE_LEVEL).getValue() == 100
                && !living.hasEffect(RevengeanceModMobEffects.RAGE_EFFECT)) {
            // 仅在服务器端添加效果
            if (!entity.level().isClientSide())
            {
                // 添加怒气效果
                living.addEffect(new
                        MobEffectInstance(RevengeanceModMobEffects.RAGE_EFFECT, 180, 0, true, true));
    ...
    handleRageAnimation(Entity entity) {
        ...
        // 获取怒气值，如果没有怒气属性则默认为0
        double rageValue =
                living.getAttributes().hasAttribute(RevengeanceModAttributes.RAGE_LEVEL) ?
                        living.getAttribute(RevengeanceModAttributes.RAGE_LEVEL).getValue() : 0;
    ...
    public static void handleRageConsumption(Player player) {
        ...
        // 检查玩家是否有怒气效果
        if (player.hasEffect(RevengeanceModMobEffects.RAGE_EFFECT)) {
            double currentRage = player.getAttribute(RevengeanceModAttributes.RAGE_LEVEL).getValue();
            ...
            if (currentRage > 0) {
                ...
                player.getAttribute(RevengeanceModAttributes.RAGE_LEVEL).setBaseValue(newRage);
                }
            }
    }
}