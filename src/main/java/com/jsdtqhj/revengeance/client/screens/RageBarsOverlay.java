...
public class RageBarsOverlay {
	private static final ResourceLocation RAGE_BAR_BASE = ResourceLocation.parse(
			RevengeanceMod.MODID + ":textures/screens/rage_bar_base.png");
	private static final ResourceLocation RAGE_BAR_FILL = ResourceLocation.parse(
	...
	static {
		for (int i = 0; i < 10; i++) {
			RAGE_FULL_ANIMATION[i] = ResourceLocation.parse(
	...
	static {
		for (int i = 0; i < 10; i++) {
			ADRENALINE_FULL_ANIMATION[i] = ResourceLocation.parse(
					RevengeanceMod.MODID + ":textures/screens/adrenaline_full_animation_" + (i + 1) + ".png");
		}
	}

	private static final ResourceLocation ADRENALINE_BAR_BASE = ResourceLocation.parse(
			RevengeanceMod.MODID + ":textures/screens/adrenaline_bar_base.png");
	private static final ResourceLocation ADRENALINE_BAR_FILL = ResourceLocation.parse(
			RevengeanceMod.MODID + ":textures/screens/adrenaline_bar_fill.png");
	private static final ResourceLocation ADRENALINE_BAR_FULL = ResourceLocation.parse(
	...
	public static void onPreGui(RenderGuiEvent.Pre event) {
		...
		double worldRage = attributes.getValue(RevengeanceModAttributes.RAGE_LEVEL);
		double worldAdren = attributes.getValue(RevengeanceModAttributes.ADRENALINE_LEVEL);
		...
		boolean hasRageEffect = player.hasEffect(RevengeanceModMobEffects.RAGE_EFFECT);
		
		// 检查是否有肾上腺素效果激活
		boolean hasAdrenalineEffect = player.hasEffect(RevengeanceModMobEffects.ADRENALINE_EFFECT);
